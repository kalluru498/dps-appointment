...
def _safe_console_stream():
    """On Windows, wrap stdout so Unicode (e.g. emoji) does not cause UnicodeEncodeError with cp1252."""
...
def setup_logger(name: str, log_file: str = None, level: int = logging.INFO) -> logging.Logger:
    ...
    # Console handler (use UTF-8-safe stream on Windows to avoid UnicodeEncodeError)
    ...
    if log_file:
        ...
        default_log_path = logs_dir / f"dps_monitor_{datetime.now().strftime('%Y%m%d')}.log"
